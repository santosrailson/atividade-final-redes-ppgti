...
- Salva /tmp/latencias_urllc.csv com latencias one-way.
"""
...
def iniciar_servidor(endereco="0.0.0.0", porta=5000, duracao_segundos=60):
    ...
    # Mantem copia de referencia no receptor
    with open("/tmp/latencias_receptor.csv", "w") as arquivo:
        arquivo.write("latencia_ms\n")
        for valor in latencias:
            arquivo.write("%.3f\n" % valor)

    print("Monitor (Scapy otimizado) finalizado. %d medicoes salvas em /tmp/latencias_urllc.csv" % len(latencias), flush=True)
